...
import { getPLReport, createExpense, createRevenue } from '../../api/api'
...
export default function FinancePage() {
  ...
  const handleAddRevenue = async (e) => {
    ...
    try {
      const res = await createRevenue({
        ...revenueForm,
        branch: selectedBranch.id,
        amount: toNum(revenueForm.amount),
      })
  ...
  const maxTrend = report
    ? Math.max(...report.trend.map((t) => Math.max(t.revenue, t.expenses, 1)))
    : 1
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            Profit & Loss, expenses and revenue reporting
          </p>
        </div>
        <div className="flex flex-col items-end gap-3">
          <div className="flex items-center gap-3">
            <select
              ...
              onChange={(e) =>
                setSelectedBranch(branches.find((b) => b.id === parseInt(e.target.value, 10)))
              }
          ...
          {report && (
            <div className="flex items-center gap-2 flex-wrap justify-end">
              ...
              <a
                href={exportUrl('/api/export/orders/', {
                  branch: selectedBranch?.id,
                  format: 'pdf',
                })}
                target="_blank"
        ...
        <>
          <div className="grid grid-cols-4 gap-4 mb-6">
            {[
              {
                label: 'Total Revenue',
                value: fmt(report.sales.total_revenue),
                sub: `${report.sales.order_count} orders`,
              ...
              {
                label: 'COGS',
                value: fmt(report.cogs),
                sub: `Gross margin ${pct(report.gross_margin_pct)}`,
                color: 'text-rose-600',
                bg: 'bg-rose-50',
                icon: '📦',
              },
              {
                label: 'Gross Profit',
                value: fmt(report.gross_profit),
                sub:
                  report.gross_profit >= 0
                    ? 'After cost of goods'
                    : '⚠ Negative margin',
                color: report.gross_profit >= 0 ? 'text-blue-600' : 'text-rose-600',
                bg: 'bg-blue-50',
                icon: '📊',
              },
              {
                ...
                sub: `Net margin ${pct(report.net_margin_pct)}`,
                color: report.net_profit >= 0 ? 'text-indigo-600' : 'text-rose-600',
                bg: 'bg-indigo-50',
                icon: report.net_profit >= 0 ? '✅' : '⚠️',
              },
            ].map((c, i) => (
              <div
                key={i}
              ...
              >
                ...
                <div
                  className={`w-11 h-11 ${c.bg} rounded-xl flex items-center justify-center text-xl shrink-0`}
                >
                  {c.icon}
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                ...
                <div className="px-5 py-4 space-y-2 text-sm">
                  <div className="flex justify-between text-gray-500">
                    ...
                    <span>{fmt(report.sales.gross_sales)}</span>
                  </div>
                  <div className="flex justify-between text-rose-500">
                    <span>Discounts</span>
                    <span>−{fmt(report.sales.discounts)}</span>
                  </div>
                  <div className="flex justify-between font-medium text-gray-700 border-t pt-2">
                    <span>Net sales</span>
                    <span>{fmt(report.sales.net_sales)}</span>
                  </div>
                  <div className="flex justify-between text-gray-500">
                    <span>Collected</span>
                    <span className="text-emerald-600">{fmt(report.sales.total_collected)}</span>
                  </div>
                  <div className="flex justify-between text-gray-500">
                    <span>Accounts receivable</span>
                    <span className="text-amber-600">{fmt(report.sales.accounts_receivable)}</span>
                  </div>
                  <div className="flex justify-between text-gray-500">
                    <span>Manual revenue</span>
                    <span>{fmt(report.sales.manual_revenue)}</span>
                  </div>
                  <div className="flex justify-between text-base font-bold text-emerald-600 border-t pt-2">
                    <span>Total revenue</span>
                    <span>{fmt(report.sales.total_revenue)}</span>
                  </div>
                </div>
              </div>
              <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                ...
                <div className="px-5 py-4 space-y-2 text-sm">
                  <div className="flex justify-between text-gray-500">
                    <span>COGS</span>
                    <span className="text-rose-600">{fmt(report.cogs)}</span>
                  </div>
                  <div className="flex justify-between font-medium text-gray-700 border-t pt-2">
                    <span>Gross profit</span>
                    <span
                      className={
                        report.gross_profit >= 0 ? 'text-blue-600' : 'text-rose-600'
                      }
                    >
                      {fmt(report.gross_profit)} ({pct(report.gross_margin_pct)})
                    </span>
                  </div>
                  <div className="flex justify-between text-gray-500 pt-1">
                    <span>Manual expenses</span>
                    <span className="text-rose-500">−{fmt(report.expenses.manual)}</span>
                  </div>
                  <div className="flex justify-between text-gray-500">
                    <span>Supplier payments</span>
                    <span className="text-rose-500">−{fmt(report.expenses.supplier_payments)}</span>
                  </div>
                  <div className="flex justify-between text-gray-500">
                    <span>Supplier outstanding</span>
                    <span className="text-amber-600">{fmt(report.expenses.supplier_outstanding)}</span>
                  </div>
                  <div className="flex justify-between font-bold text-base border-t pt-2">
                    <span>Net profit</span>
                    <span
                      className={
                        report.net_profit >= 0 ? 'text-indigo-600' : 'text-rose-600'
                      }
                    >
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-6">
                  ...
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full flex items-end gap-1 h-40">
                      <div
                        ...
                        style={{
                          height: maxTrend > 0 ? (t.revenue / maxTrend) * 100 + '%' : '2%',
                        }}
                        title={`Revenue: ${fmt(t.revenue)}`}
                      />
                      <div
                        className="flex-1 bg-rose-400 rounded-t-md transition-all"
                        style={{
                          height: maxTrend > 0 ? (t.expenses / maxTrend) * 100 + '%' : '2%',
                        }}
              ...
              <div className="flex gap-4 mt-4 justify-center text-xs text-gray-500">
                <div className="flex items-center gap-1">
                  <span className="w-3 h-3 bg-emerald-400 rounded-sm" /> Revenue
                </div>
                <div className="flex items-center gap-1">
                  <span className="w-3 h-3 bg-rose-400 rounded-sm" /> Expenses
                </div>
              </div>
            </div>
          )}
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                <p className="text-sm text-gray-500">
                  Total:{' '}
                  <span className="font-semibold text-rose-600">{fmt(report.expenses.manual)}</span>
                </p>
                <button
          ...
          {tab === 'revenue' && (
            <div>
              <div className="flex justify-between items-center mb-4">
                <p className="text-sm text-gray-500">
                  ...
                  <span className="font-semibold text-emerald-600">
                    {fmt(report.sales.manual_revenue)}
                  </span>
                </p>
                <button
              ...
              <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                {revenues.length === 0 ? (
                  <p className="p-8 text-center text-gray-400 text-sm">No manual revenue recorded</p>
                ) : (
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wide">
                      <tr>
